...
- # 주소
주소는 송신지를 특정하는 정보를 의미한다
네트워크에 흐르는 수많은 패킷에는 모두 송수신지의 정보를 담고 있고,
IP주소, MAC 주소 모두 네트워크에서 사용되는 주소이다.


- 유니캐스트
가장 일반적인 형태의 송수신 방식이며 하나의 수신지에 메시지를 전송하는 방식이다
전송 방식은 송신지와 수신지가 일대일로 메시지를 주고 받는 경우

- 브로캐스트
자신을 제외한 네트워크상의 모든 호스트에게 전송하는 방식이며 브로캐스트가 전송되는 범위를
'브로캐스트 도메인'이라 한다.

# 프로토콜
프로토콜이란 노드 간에 정보를 올바르게 주고받기 위해 합의된 규칙이나 방법을 의미한다
즉, 서로 다른 통신 장치들이 정보를 주고받으려면 프로토콜이 통해야 한다.

인터넷을 이용하는 것도, 이메일, 파일을 주고 받는 것도 모두 상대 호스트와 동일한 프로토콜을 사용하기 때문
